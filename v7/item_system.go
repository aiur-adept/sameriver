...
func (i *ItemSystem) GetComponentDeps() []any {
	...
	deps := []any{}
	if i.spawn {
		deps = append(deps, ITEM_, ITEM, "ITEM")
		deps = append(deps, POSITION_, VEC2D, "POSITION")
		deps = append(deps, BOX_, VEC2D, "BOX")
	}
	if i.sprite {
		deps = append(deps, BASESPRITE_, SPRITE, "BASESPRITE")
	}
	if i.despawn_ms != nil {
		deps = append(deps, DESPAWNTIMER_, TIMEACCUMULATOR, "DESPAWNTIMER")
	}
	return deps
}
